 ...
 */
TinyCore.Module.register( 'todo_summary', function ( oSandBox )
{
	...
	return {
		...
		onStart : function ( oStartData )
		{
			...
			oSandBox.subscribe( 'todo:update', function ( oTopic )
			{
				console.info(oTopic.name, oTopic.data);
				self.incCount( oDone, oTopic.data.done ? +1 : -1 );
			} );

			oSandBox.subscribe( 'todo:remove', function ( oTopic )
			{
				console.info(oTopic.name, oTopic.data);
				self.incCount( oTotal, -1 );
		...
		onClearListClick : function ( eEvent )
		{
			...
			oSandBox.publish( 'list:clear' ); // Let's request some cleanup...
		},
		...
		{
			eEvent.preventDefault();
			oSandBox.publish( 'list:clean' ); // Let's request some cleanup...
		}
	};	
}, 'domlib_sandbox' );